...
// Admin: Reset and re-seed questions with proper UTF-8 encoding
func ResetQuestions(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Set UTF-8 encoding header
		c.Header("Content-Type", "application/json; charset=utf-8")

		// Delete all existing questions
		if err := db.Exec("DELETE FROM questions").Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to clear questions"})
			return
		}

		// Reset auto increment
		if err := db.Exec("ALTER TABLE questions AUTO_INCREMENT = 1").Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to reset auto increment"})
			return
		}

		// Re-seed with proper UTF-8 questions
		...
		// Insert new questions with proper UTF-8
		...
		c.JSON(http.StatusOK, gin.H{
			"message": "Questions reset successfully with proper UTF-8 encoding",
			"count": len(questions),
		})
	}
}